...
test('keeps the responsive companion bounded and non-blocking', async () => {
  ...
  assert.match(component, /width:\s*clamp\(220px,\s*22vw,\s*320px\)/)
  assert.match(component, /max-height:\s*42vh/)
  assert.match(component, /width:\s*min\(42vw,\s*190px\)/)
  assert.match(component, /max-height:\s*32vh/)
