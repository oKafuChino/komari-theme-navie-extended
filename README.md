...
5. 在主主题设置中开启 Live2D，并填写模型入口，例如 `/themes/komari-live2d-models/dist/model/model.model3.json`。旧版 Komari 可填写 `/theme/komari-live2d-models/dist/model/model.model3.json`；主题会在首选入口不可用时尝试对应的兼容路径。嵌套模型目录同样可用。

Komari 1.2.6+ 通过原生静态路由 `/themes/:id/*path` 提供资源包文件；旧版使用 `/theme/:id/*path`。因此 Docker、面板、systemd 和反向代理部署不需要额外路径规则。主主题更新只替换 `NaiveExtended`，不会覆盖 `komari-live2d-models`。
