...
常规 `pnpm build` 只在本地 `release/` 目录生成主主题 ZIP：

| 文件                                          | 用途                   |
| --------------------------------------------- | ---------------------- |
| `komari-theme-naive-extended-build-<sha>.zip` | 主主题，上传并启用它。 |

从 [Releases](https://github.com/oKafuChino/komari-theme-navie-extended/releases) 下载主主题 ZIP。发布者上传 GitHub Release 时，只上传主主题 ZIP；Komari 的仓库更新机制会下载最新 Release 的第一个附件。
...
1. 仅首次安装时，由发布者执行 `pnpm build:model-pack` 生成并提供 `komari-live2d-model-pack-template.zip`，然后下载并解压。
...
5. 在主主题设置中开启 Live2D，并填写当前 Komari 实际可访问的模型入口：新版本使用 `/themes/komari-live2d-models/dist/model/model.model3.json`，旧版使用 `/theme/komari-live2d-models/dist/model/model.model3.json`。嵌套模型目录同样可用。

Komari 1.2.6+ 通过原生静态路由 `/themes/:id/*path` 提供资源包文件；旧版使用 `/theme/:id/*path`。因此 Docker、面板、systemd 和反向代理部署不需要额外路径规则。模型包保持独立的 `1.0.0` 标识，常规主主题构建、CI 与 Release 不会生成或上传它；主主题更新只替换 `NaiveExtended`，不会覆盖 `komari-live2d-models`。
...
pnpm lint
pnpm build
pnpm build:model-pack
```

`pnpm build` 会执行类型检查、生产构建，并验证主主题 ZIP。`pnpm build:model-pack` 仅用于首次安装或管理员主动重建 Live2D 模型包模板，不能随主主题 Release 上传。可用 `pnpm preview` 预览生产构建。
